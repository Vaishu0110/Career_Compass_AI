...
export async function generateResumePDFKit(res, analysis, user) {
    ...
    doc.fontSize(26).fillColor("#2563EB").text("Career Compass AI", {
        align: "center",
    });

    doc.moveDown(2);

    doc.fontSize(12).fillColor("black").text(`Candidate: ${user?.name || "Unknown"}`);

    doc.text(`Target Role: ${user?.targetRole || "Not Specified"}`);

    doc.moveDown();

    doc.fontSize(14).fillColor("#16A34A").text(`Resume Score: ${analysis.resumeScore || 0}`);

    doc.fillColor("#2563EB").fontSize(14).text(`ATS Score: ${analysis.atsScore || 0}`);
